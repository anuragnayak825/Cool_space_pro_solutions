    ...
    <div className="w-full bg-white py-20 px-4 text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-10 max-w-6xl mx-auto">
        {stats.map((item, index) => (
          <div
            key={index}
            className="relative flex flex-col items-center justify-center p-6 rounded-lg shadow-md bg-white"
          >
            <div className="absolute -top-8 w-20 h-20 bg-white shadow-md border-b-2 border-[#00C4CC] flex justify-center items-center rounded-full">
              <div className="w-10 h-10 object-contain filter invert-[40%] brightness-[0.7] contrast-[1.2] flex justify-center items-center">
                {item.icon2}
              </div>
            </div>
